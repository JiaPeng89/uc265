...
do
  ...
  do
    for kQp in 0 1 2 3
    do
      $EXE --input $inputDir/${arrayInput[$jInput]} --input-res ${arrayRes[$jInput]} --fps ${arrayFps[$jInput]} -f ${arrayNumFrame[$jInput]} -p $speed --crf ${arrayQP[kQp]} --psnr --ssim --tune=psnr --keyint=-1 -o $speedDir/${arrayInput[$jInput]}_set$kQp.bin
    done
  done
done
